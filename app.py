...
def pre_clean(file):
    ...
    for i in range(min(3, len(df))):
        row = df.iloc[i].astype(str).str.cat(sep=" ")
        if any(k in row for k in ["상품코드", "품번", "재고", "PO", "세대"]):
...
for f in files:
    ...
    elif "시판" in cols and "품번" in cols:
        data["market"] = df

    elif "상품코드" in cols and "상품명" in cols:
        data["item"] = df
